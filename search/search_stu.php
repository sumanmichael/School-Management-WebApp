	...
	<div class="ui bottom attached segment" >
	  	<div class="ui visible fixed inverted left vertical sidebar menu" id="sidebar" style="padding-top: 4%">
	  		<div class="ui styled inverted fluid accordion item">
				...
				<div>
				    <a class="active item" href="../search/search_stu.php">
				      <b>Search</b>
	  	...
	  	<div class="pusher" style="position: relative;left:-20%; top: 5%; width:150%;" >
	  		<div class="ui raised very padded text container segment" style="position: relative; left: -10%; bottom:40%;">
				...
				<form class="ui form" method="post" action="#">
					<div class="eleven wide field" style="position: relative;left:15%; padding-bottom: 5%">	
						<div class="ui action input">
							<input name="input" placeholder="Search..." type="text">
							<div class="ui compact selection dropdown">
							  <input name="column" type="hidden">
							...
							<button name="show" type="submit" class="ui blue button">Search</button>
						</div>
					</div>
				</form>

			  	
					<div class="ui segment" style="overflow-x: auto; display: none;" id="table">
						<table class="ui definition collapsing table">
							...
							<tbody>
								<?php
									error_reporting(E_ALL & ~E_NOTICE);
									include_once('../service/dbcon.php');
									if (isset($_POST['show']) || isset($_POST['generate'])|| isset($_POST['menu']) ) {
										$column = $_POST['column'];
										$input = $_POST['input'];

										$query = "SELECT * FROM stu_data where $column = '$input' order by class,section,gender,name";
						...
						<?php
							if (isset($_POST['show']) || isset($_POST['menu'])) {
								echo '<script type="text/javascript">document.getElementById("table").style.display = "";</script>';
							}
						?>
					</div>
						<form class="ui form" method="post" action="#">
							<input type="hidden" name="column" value="<?php error_reporting(E_ALL & ~E_NOTICE); echo $_POST['column']; ?>">
							<input type="hidden" name="input" value="<?php error_reporting(E_ALL & ~E_NOTICE); echo $_POST['input']; ?>">
							...
							<div class="fields" >
						  		...
						  		<?php
									...
									if (isset($_POST['generate'])) {
										...
										$query = "SELECT * FROM stu_data where $column = '$input' order by class,section,gender,name";
										$fname = '../output/SearchListOf-'.$column.'_by_'.$input.'-'.date('dMY').'.csv';
										$res = mysqli_query($con,$query);
										$fp = fopen($fname,'w');
