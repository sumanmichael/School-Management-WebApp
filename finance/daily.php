	  		...
	  		<div class="ui styled inverted fluid accordion item">
				...
				<div>
				    ...
				    <div class="active content">
				    	<a href="../finance/daily.php" class="active item"><i class="calendar icon"></i>Daily A/c</a>
	  		...
	  		<div class="ui raised very padded text container segment" style="position: relative; left: -10%; bottom:40%;">
				...
				<h2 class="ui center aligned icon header">
					<i class="calendar icon"></i>
				  		 Day-wise Financial List
				</h2>
				<form class="ui form" method="post" action="#">
					<div class="fields" style="position: relative;left: -3%">
						<div class="four wide field" style="position: relative; left:22%"><label>Year</label><input type="number" name="year" value="<?php echo(date('Y'));?>"></div>
						<div class="three wide field" style="position: relative; left:23%"><label>Month</label><input type="number" name="month" value="<?php echo(date('m'));?>"></div>
						<div class="three wide field" style="position: relative; left:24%"><label>Day</label><input type="number" name="day" value="<?php echo(date('d'));?>"></div>
					</div>
				...
				<div class="ui segments" style="display: none; " id="table">
					<div class="ui segment">
	                        <h3 class="ui header" style="padding-left:30%">
	                            Day-wise Financial Details - <?php echo $_POST['day'].'-'.$_POST['month'].'-'.$_POST['year']; ?>
	                        </h3>
	                </div>
	                <div class="ui segment" style="overflow-x: auto;">
							...
							<tbody>
								<?php
									...
									if (isset($_POST['show']) || isset($_POST['generate'])) {
										...
										$query = "SELECT * FROM fee_rcpt WHERE YEAR(date_time)=$year AND MONTH(date_time)=$month AND DAYOFMONTH(date_time) = $day";
										...
										$query2 = "SELECT COUNT(*) count, sum(amount) sum, round(avg(amount),2) avg FROM fee_rcpt WHERE YEAR(date_time)=$year AND MONTH(date_time)=$month AND DAYOFMONTH(date_time) = $day";
						...
						<form class="ui form" method="post" action="#">
							...
							<input type="hidden" name="month" value="<?php error_reporting(E_ALL & ~E_NOTICE); echo $_POST['month']; ?>">
							<input type="hidden" name="day" value="<?php error_reporting(E_ALL & ~E_NOTICE); echo $_POST['day']; ?>">
							<div class="fields" >
						  		...
						  		<?php
									...
									if (isset($_POST['generate'])) {
										...
										$query = "SELECT * FROM fee_rcpt WHERE YEAR(date_time)=$year AND MONTH(date_time)=$month AND DAYOFMONTH(date_time) = $day";
										$res = mysqli_query($con,$query);
										$fname = '../output/FinanceListOf-'.$month.'-'.$year.'-'.date('dMY').'.csv';
